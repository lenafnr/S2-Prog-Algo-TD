...
std::unordered_map<IDType, std::pair<float, IDType>> Dijkstra(WeightedGraph const &graph, IDType const start, IDType const end)
{
    ...
    while (!to_visit.empty())
    {
        ...
        for (auto edge : graph.get_neighbors(current_node))
        {
            float cost = current_cost + edge.weight;

            if (!distances.contains(edge.to) || cost < distances[edge.to].first)
            {
                distances[edge.to] = {cost, edge.to};
                to_visit.push({cost, edge.to});
            }
        }
    }
